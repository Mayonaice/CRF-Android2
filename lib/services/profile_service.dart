...
class ProfileService {
  ...
  // Cache manager untuk menyimpan gambar
  ...
  Future<Uint8List> _compressImageIfNeeded(Uint8List imageBytes) async {
    if (imageBytes.length <= maxImageSize) {
      debugPrint('Image size is already under 2MB: ${imageBytes.length} bytes');
      return imageBytes;
    }
    
    debugPrint('Compressing image from ${imageBytes.length} bytes');
    ...
    try {
      ...
      // Save to temporary file for compression
      ...
      final result = await FlutterImageCompress.compressWithFile(
        tempFile.path,
        minWidth: width ~/ 2, // Reduce dimensions if very large
        minHeight: height ~/ 2,
        quality: quality,
      );
      ...
      if (result == null) {
        debugPrint('Compression failed, returning original image');
        return imageBytes;
      }
      
      debugPrint('Image compressed to ${result.length} bytes (${(result.length / imageBytes.length * 100).toStringAsFixed(1)}% of original)');
      return result;
    } catch (e) {
  ...
  // Get profile photo from API with caching
  Future<ImageProvider> getProfilePhoto() async {
    try {
      ...
      try {
        ...
        if (fileInfo != null) {
          debugPrint('Found image in disk cache for $employeeCode');
          final cachedImage = MemoryImage(await fileInfo.file.readAsBytes());
          // Store in memory cache
          _imageCache[cacheKey] = cachedImage;
          return cachedImage;
        }
      } catch (e) {
        debugPrint('Error checking disk cache: $e');
      }
      ...
      try {
        final directUrl = 'http://10.10.0.223/LocalCRF/api/CRF/photo/local/$formattedEmployeeCode';
        ...
        if (directResponse.statusCode == 200 && directResponse.bodyBytes.isNotEmpty) {
          ...
          // Compress image if needed
          final compressedBytes = await _compressImageIfNeeded(directResponse.bodyBytes);
          ...
          final memoryImage = MemoryImage(compressedBytes);
      ...
      final response = await http.get(
        Uri.parse('$baseUrl/CRF/photo/local/$formattedEmployeeCode'),
        headers: {
          'Authorization': 'Bearer $token',
          'Content-Type': 'application/json',
        },
      ...
      if (response.statusCode == 200) {
        ...
        if (response.bodyBytes.isNotEmpty) {
          ...
          // Compress image if needed
          final compressedBytes = await _compressImageIfNeeded(response.bodyBytes);
          
          // Save to cache
          final fallbackUrl = '$baseUrl/CRF/photo/local/$formattedEmployeeCode';
          await cacheManager.putFile(
            fallbackUrl,
            compressedBytes,
            key: cacheKey,
